...
class TestServiceOrderbookCancel(OrderbookWrapper):
    """
    Order cancellation tests for OrderbookService.

    This test suite validates order cancellation functionality:
    - Cancelling unfilled orders in backtest mode
    - Cancelling unfilled orders in production mode
    - Closing orders with zero execution (delegates to cancel)
    - Closing partially filled orders (cancel unfilled + close filled)
    - Closing fully filled orders (normal close)
    - Margin release on cancellation
    - Transaction callbacks on cancellation
    """

    def test_cancel_order_in_backtest_mode_releases_margin(self) -> None:
        """Verify cancelling an order in backtest mode releases reserved margin."""
    ...
    def test_cancel_order_calls_transaction_callback(self) -> None:
        """Verify on_transaction callback is called when order is cancelled."""
        tick = self._create_tick(50000.0)
        self._orderbook.refresh(tick)
    ...
    def test_cancel_order_without_tick_fails(self) -> None:
        """Verify order cancellation fails when tick is not set."""
        order = self._create_order()
    ...
    def test_cancel_order_not_in_orderbook_fails(self) -> None:
        """Verify cancelling an order not in orderbook fails."""
        tick = self._create_tick(50000.0)
        self._orderbook.refresh(tick)
    ...
    def test_cancel_order_in_closing_status_fails(self) -> None:
        """Verify cancelling an order in CLOSING status fails."""
        tick = self._create_tick(50000.0)
        self._orderbook.refresh(tick)
    ...
    def test_cancel_order_in_closed_status_fails(self) -> None:
        """Verify cancelling an order in CLOSED status fails."""
        tick = self._create_tick(50000.0)
        self._orderbook.refresh(tick)
    ...
    def test_cancel_order_removes_from_open_orders_index(self) -> None:
        """Verify cancelling an order removes it from open orders index."""
        tick = self._create_tick(50000.0)
        self._orderbook.refresh(tick)
    ...
    def test_close_order_with_zero_executed_volume_cancels(self) -> None:
        """Verify closing order with zero executed_volume calls cancel instead."""
        tick = self._create_tick(50000.0)
        self._orderbook.refresh(tick)
    ...
    def test_close_partially_filled_order_releases_unfilled_margin(self) -> None:
        """Verify closing partially filled order releases margin for unfilled portion."""
        tick = self._create_tick(50000.0)
        self._orderbook.refresh(tick)
    ...
    def test_close_partially_filled_order_updates_volume_to_executed(self) -> None:
        """Verify closing partially filled order updates volume to executed_volume."""
        tick = self._create_tick(50000.0)
        self._orderbook.refresh(tick)
    ...
    def test_close_fully_filled_order_works_normally(self) -> None:
        """Verify closing fully filled order works as expected (normal flow)."""
        tick = self._create_tick(50000.0)
        self._orderbook.refresh(tick)
    ...
    def test_cancel_order_updates_timestamp(self) -> None:
        """Verify cancelling an order updates the order's updated_at timestamp."""
        tick = self._create_tick(50000.0)
        self._orderbook.refresh(tick)
