...
class TestServiceOrderbookSecurity(OrderbookWrapper):
    ...
    def test_async_get_trades_exception_handled(self) -> None:
        ...
        with patch.object(orderbook._gateway_handler._gateway, 'get_trades', side_effect=Exception('Simulated get_trades error')):
            gateway_order = GatewayOrderModel(id='gw-123', symbol='BTCUSDT', side=OrderSide.BUY, price=50000.0, executed_volume=0.01, status=GatewayOrderStatus.EXECUTED)
    ...
    def test_validate_volume_below_minimum_rejected(self) -> None:
        orderbook = self._create_orderbook()
        orderbook._backtest = False
        symbol_info = GatewaySymbolInfoModel(symbol='BTCUSDT', min_quantity=0.001, max_quantity=1000.0, min_price=0.01, max_price=1000000.0, min_notional=10.0)
    ...
    def test_validate_notional_below_minimum_rejected(self) -> None:
        orderbook = self._create_orderbook()
        orderbook._backtest = False
        symbol_info = GatewaySymbolInfoModel(symbol='BTCUSDT', min_quantity=0.001, max_quantity=1000.0, min_price=0.01, max_price=1000000.0, min_notional=100.0)
        with patch.object(orderbook._gateway_handler._gateway, 'get_symbol_info', return_value=symbol_info):
    ...
    def test_concurrent_open_orders_no_over_leverage(self) -> None:
        ...
        total_margin_used = sum((order.volume * order.price / orderbook._leverage for order in results if order.status == OrderStatus.OPEN))
