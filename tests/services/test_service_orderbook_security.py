...
class TestServiceOrderbookSecurity(OrderbookWrapper):
    """
    Security and safety tests for OrderbookService.

    This test suite validates security improvements:
    - Exception handling in async operations
    - Input validation for gateway operations
    - Thread safety with concurrent access
    - Production order safety with gateway validation
    """
    ...
    def test_async_get_trades_exception_handled(self) -> None:
        """Verify exception in get_trades doesn't prevent order completion."""
        ...
        with patch.object(
            orderbook._gateway_handler._gateway,
            "get_trades",
            side_effect=Exception("Simulated get_trades error"),
        ):
    ...
    def test_validate_empty_symbol_rejected(self) -> None:
        """Verify orders with empty symbols are rejected."""
    ...
    def test_validate_symbol_too_long_rejected(self) -> None:
        """Verify symbols exceeding max length are rejected."""
        orderbook = self._create_orderbook()
        orderbook._backtest = False
    ...
    def test_validate_volume_below_minimum_rejected(self) -> None:
        """Verify orders with volume below minimum are rejected."""
        orderbook = self._create_orderbook()
        orderbook._backtest = False
    ...
    def test_validate_volume_above_maximum_rejected(self) -> None:
        """Verify orders with volume above maximum are rejected."""
        orderbook = self._create_orderbook()
        orderbook._backtest = False
    ...
    def test_validate_price_below_minimum_rejected(self) -> None:
        """Verify orders with price below minimum are rejected."""
        orderbook = self._create_orderbook()
        orderbook._backtest = False
    ...
    def test_validate_price_above_maximum_rejected(self) -> None:
        """Verify orders with price above maximum are rejected."""
        orderbook = self._create_orderbook()
        orderbook._backtest = False
    ...
    def test_validate_notional_below_minimum_rejected(self) -> None:
        """Verify orders with notional value below minimum are rejected."""
        orderbook = self._create_orderbook()
        orderbook._backtest = False
    ...
    def test_validate_valid_order_accepted(self) -> None:
        """Verify orders within all limits are accepted."""
        orderbook = self._create_orderbook()
        orderbook._backtest = False
    ...
    def test_symbol_info_cached_after_first_request(self) -> None:
        """Verify symbol info is cached to reduce gateway calls."""
        orderbook = self._create_orderbook()
        orderbook._backtest = False
    ...
    def test_concurrent_open_orders_no_over_leverage(self) -> None:
        """Verify concurrent opens maintain balance integrity."""
        ...
        total_margin_used = sum(
            (order.volume * order.price) / orderbook._leverage
    ...
    def test_concurrent_balance_operations_consistent(self) -> None:
        """Verify concurrent open/close operations maintain balance consistency."""
    ...
    def test_concurrent_property_access_no_deadlock(self) -> None:
        """Verify concurrent property access doesn't cause deadlock."""
        orderbook = self._create_orderbook(balance=10000.0)
        tick = self._create_tick(50000.0)
        orderbook.refresh(tick)
    ...
    # ═══════════════════════════════════════════════════════════
    # PRODUCTION ORDER SAFETY TESTS
    # ═══════════════════════════════════════════════════════════

    def test_open_order_gateway_failure_reverts_balance(self) -> None:
        """Verify balance is reverted when gateway fails to place order."""
        orderbook = self._create_orderbook(balance=10000.0)
        orderbook._backtest = False
    ...
    def test_close_order_gateway_failure_reverts_balance(self) -> None:
        """Verify balance is reverted when gateway fails to close order."""
        orderbook = self._create_orderbook(balance=10000.0)
        orderbook._backtest = False
    ...
    def test_multiple_gateway_failures_tracked_correctly(self) -> None:
        """Verify multiple gateway failures don't corrupt orderbook state."""
        orderbook = self._create_orderbook(balance=10000.0)
        orderbook._backtest = False
