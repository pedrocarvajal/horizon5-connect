...
class HorizonRouterWrapper(unittest.TestCase):
    ...
    def _create_test_backtest_payload(
        self,
        ...
        settings = BacktestSettingsModel(
            asset=asset or self._DEFAULT_BACKTEST_ASSET,
            strategies=strategies or "default_strategy",
            from_date=from_date or self._DEFAULT_BACKTEST_FROM_DATE,
            to_date=to_date or self._DEFAULT_BACKTEST_TO_DATE,
        )
        return BacktestCreateModel(
            user_id=user_id or self._DEFAULT_USER_ID, status=BacktestStatus(status), settings=settings
        )
