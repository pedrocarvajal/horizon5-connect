...
class BinanceWrapper(unittest.TestCase):
    ...
    def _place_test_order(self, symbol: Optional[str]=None, side: OrderSide=OrderSide.BUY, volume: Optional[float]=None) -> Optional[GatewayOrderModel]:
        if symbol is None:
            symbol = self._SYMBOL
        if volume is None:
    ...
    def _validate_sandbox_configuration(self) -> None:
        ...
        assert sandbox_enabled is True, 'BINANCE_TESTNET must be enabled for tests. Set BINANCE_TESTNET=True in environment variables'
        testnet_api_key = get_env('BINANCE_TESTNET_API_KEY')
        testnet_api_secret = get_env('BINANCE_TESTNET_API_SECRET')
        assert testnet_api_key is not None, 'BINANCE_TESTNET_API_KEY must be configured for tests. Set BINANCE_TESTNET_API_KEY in environment variables'
        assert testnet_api_key != '', 'BINANCE_TESTNET_API_KEY must not be empty. Set BINANCE_TESTNET_API_KEY in environment variables'
        assert testnet_api_secret is not None, 'BINANCE_TESTNET_API_SECRET must be configured for tests. Set BINANCE_TESTNET_API_SECRET in environment variables'
        assert testnet_api_secret != '', 'BINANCE_TESTNET_API_SECRET must not be empty. Set BINANCE_TESTNET_API_SECRET in environment variables'

    def _compute_close_parameters(self, order: GatewayOrderModel, position: GatewayPositionModel) -> Tuple[OrderSide, float]:
    ...
    def _place_close_order(self, symbol: str, order: GatewayOrderModel, close_side: OrderSide, close_volume: float) -> None:
        close_order = self._gateway.place_order(symbol=symbol, side=close_side, order_type=OrderType.MARKET, volume=close_volume)
