...
class TestBinanceSymbol(BinanceWrapper):
    ...
    def test_get_leverage_info(self) -> None:
        order = self._gateway.place_order(symbol=self._SYMBOL, side=OrderSide.BUY, order_type=OrderType.MARKET, volume=self._DEFAULT_ORDER_VOLUME)
    ...
    def _assert_symbol_info_is_valid(self, symbol_info, expected_symbol: Optional[str]=None) -> None:
        ...
        assert symbol_info.price_precision >= 0, f'Price precision should be >= 0, got {symbol_info.price_precision}'
        assert symbol_info.quantity_precision >= 0, f'Quantity precision should be >= 0, got {symbol_info.quantity_precision}'
    ...
    def _assert_trading_fees_is_valid(self, trading_fees, expected_symbol: Optional[str]=None) -> None:
        ...
        assert trading_fees.maker_commission >= 0, f'Maker commission should be >= 0, got {trading_fees.maker_commission}'
        assert trading_fees.taker_commission is not None, 'Taker commission should not be None'
        assert trading_fees.taker_commission >= 0, f'Taker commission should be >= 0, got {trading_fees.taker_commission}'
    ...
    def _assert_leverage_info_is_valid(self, leverage_info, expected_symbol: Optional[str]=None) -> None:
        ...
        assert leverage_info.symbol == expected_symbol, f'Symbol should be {expected_symbol}, got {leverage_info.symbol}'
