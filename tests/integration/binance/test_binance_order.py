...
class TestBinanceOrder(BinanceWrapper):
    ...
    def test_get_orders(self) -> None:
        ...
        orders = self._gateway.get_orders(symbol=self._SYMBOL, pair=self._SYMBOL, order_id=1, start_time=start_time, end_time=end_time, limit=100)
    ...
    def _assert_order_is_valid(self, order: Optional[GatewayOrderModel], expected_type: OrderType, expected_symbol: Optional[str]=None) -> None:
        ...
        if expected_type.is_market():
            assert order.status in [GatewayOrderStatus.PENDING, GatewayOrderStatus.EXECUTED], 'Status should be PENDING or EXECUTED'
