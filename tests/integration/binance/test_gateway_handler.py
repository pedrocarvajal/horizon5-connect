...
class TestGatewayHandler(BinanceWrapper):
    ...
    def test_open_order_market_with_polling(self) -> None:
        order = self._build_order_model(symbol=self._SYMBOL, side=OrderSide.BUY, order_type=OrderType.MARKET, volume=self._DEFAULT_ORDER_VOLUME)
        ...
        self._wait_for_order_status(order=order, target_status=OrderStatus.OPEN, timeout_seconds=self._POLLING_TIMEOUT_SECONDS, handler=self._handler)
    ...
    def test_open_order_backtest_mode(self) -> None:
        handler_backtest = GatewayHandlerService(gateway=self._gateway, backtest=True, backtest_id=self._DEFAULT_ORDER_BACKTEST_ID)
        order = self._build_order_model(symbol=self._SYMBOL, side=OrderSide.BUY, order_type=OrderType.MARKET, volume=self._DEFAULT_ORDER_VOLUME, backtest=True)
    ...
    def test_open_order_with_invalid_gateway_response(self) -> None:
        order = self._build_order_model(symbol=self._DEFAULT_ORDER_INVALID_SYMBOL, side=OrderSide.BUY, order_type=OrderType.MARKET, volume=self._DEFAULT_ORDER_INVALID_VOLUME)
        result = self._handler.open_order(order)
    ...
    def test_close_order_buy_position_with_polling(self) -> None:
        ...
        if order.status != OrderStatus.CLOSED:
            self._wait_for_order_status(order=order, target_status=OrderStatus.CLOSED, timeout_seconds=self._POLLING_TIMEOUT_SECONDS, handler=self._handler)
    ...
    def test_close_order_sell_position_with_polling(self) -> None:
        order = self._build_order_model(symbol=self._SYMBOL, side=OrderSide.SELL, order_type=OrderType.MARKET, volume=self._DEFAULT_ORDER_VOLUME)
        open_result = self._handler.open_order(order)
        assert open_result is True, 'Order should open successfully'
        self._wait_for_order_status(order=order, target_status=OrderStatus.OPEN, timeout_seconds=self._POLLING_TIMEOUT_SECONDS, handler=self._handler)
        assert order.status == OrderStatus.OPEN, f'Order should be OPEN before closing, got {order.status}'
    ...
    def _build_order_model(self, symbol: str, side: OrderSide, order_type: OrderType, volume: float, price: float=0.0, backtest: bool=False) -> OrderModel:
        return OrderModel(symbol=symbol, side=side, order_type=order_type, volume=volume, price=price, backtest=backtest, gateway=self._gateway, created_at=datetime.datetime.now(tz=TIMEZONE), updated_at=datetime.datetime.now(tz=TIMEZONE))

    def _wait_for_order_status(self, order: 'OrderModel', target_status: OrderStatus, timeout_seconds: int=70, handler: Optional[GatewayHandlerService]=None) -> None:
