...
class TestExecuteRequest(unittest.TestCase):
    ...
    def _create_mock_response(self, status_code: int, json_data: Optional[Dict[str, Any]]=None, text: str='OK') -> MagicMock:
    ...
    def test_execute_request_returns_json_on_get_success(self, mock_get: MagicMock) -> None:
        mock_response = self._create_mock_response(status_code=HttpStatus.OK.value, json_data={'data': 'success'})
        mock_get.return_value = mock_response
        result = execute_request(method='GET', url=self._TEST_URL, api_key=self._TEST_API_KEY, api_secret=self._TEST_API_SECRET, params={'symbol': self._TEST_SYMBOL})
    ...
    def test_execute_request_returns_json_on_post_success(self, mock_post: MagicMock) -> None:
        mock_response = self._create_mock_response(status_code=HttpStatus.OK.value, json_data={'orderId': '123'})
        mock_post.return_value = mock_response
        result = execute_request(method='POST', url=self._TEST_URL, api_key=self._TEST_API_KEY, api_secret=self._TEST_API_SECRET)
        assert result is not None
        assert result == {'orderId': '123'}

    @patch('services.gateway.gateways.binance.helpers.execute_request.requests.get')
    def test_execute_request_returns_none_on_http_error(self, mock_get: MagicMock) -> None:
        mock_response = self._create_mock_response(status_code=HttpStatus.BAD_REQUEST.value, text='Bad Request')
        mock_get.return_value = mock_response
        result = execute_request(method='GET', url=self._TEST_URL, api_key=self._TEST_API_KEY, api_secret=self._TEST_API_SECRET)
    ...
    def test_execute_request_calls_log_error_callback_on_failure(self, mock_get: MagicMock) -> None:
        ...
        result = execute_request(method='GET', url=self._TEST_URL, api_key=self._TEST_API_KEY, api_secret=self._TEST_API_SECRET, log_error=log_error)
        assert result is None
        assert log_error_called is True

    def test_execute_request_validates_required_parameters(self) -> None:
        test_cases = [('empty_api_key', '', self._TEST_API_SECRET, self._TEST_URL, 'GET'), ('empty_api_secret', self._TEST_API_KEY, '', self._TEST_URL, 'GET'), ('empty_url', self._TEST_API_KEY, self._TEST_API_SECRET, '', 'GET'), ('unsupported_method', self._TEST_API_KEY, self._TEST_API_SECRET, self._TEST_URL, 'PATCH')]
