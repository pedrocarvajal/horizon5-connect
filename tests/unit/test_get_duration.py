...
class TestGetDuration(unittest.TestCase):
    ...
    def test_get_duration_boundary_minute_to_hour(self) -> None:
        end_59_minutes = self._BASE_DATETIME + datetime.timedelta(
            minutes=59,
            seconds=59,
        )
        end_60_minutes = self._BASE_DATETIME + datetime.timedelta(hours=1)

        result_59_minutes = get_duration(
            self._BASE_DATETIME,
            end_59_minutes,
        )
        result_60_minutes = get_duration(
            self._BASE_DATETIME,
            end_60_minutes,
        )
    ...
    def test_get_duration_boundary_hour_to_day(self) -> None:
        end_23_hours = self._BASE_DATETIME + datetime.timedelta(
            hours=23,
            minutes=59,
            seconds=59,
        )
        ...
        result_23_hours = get_duration(
            self._BASE_DATETIME,
            end_23_hours,
        )
        result_24_hours = get_duration(
            self._BASE_DATETIME,
            end_24_hours,
        )
