...
DAYS_IN_YEAR = 365
DEFAULT_BACKTEST_EXPECTATION = BacktestExpectationModel.default()


def get_quality(
    method: QualityMethod = QualityMethod.FQS,
    expectations: BacktestExpectationModel = DEFAULT_BACKTEST_EXPECTATION,
    days_elapsed: int = 0,
    **metrics: Any,
) -> Tuple[float, str]:
    ...
    if method == QualityMethod.FQS:
        return _calculate_fqs(metrics, expectations, days_elapsed)

    if method == QualityMethod.DRAWDOWN:
        return _calculate_single_metric(metrics, expectations, "max_drawdown", "drawdown", higher_is_better=False)

    if method == QualityMethod.PROFIT_FACTOR:
        return _calculate_single_metric(metrics, expectations, "profit_factor", "profit_factor")

    if method == QualityMethod.R_SQUARED:
        return _calculate_single_metric(metrics, expectations, "r_squared", "r_squared")

    if method == QualityMethod.WIN_RATIO:
        return _calculate_single_metric(metrics, expectations, "win_ratio", "win_ratio")
...
def _calculate_single_metric(
    ...
    expectation_range = expectations.get_range(metric_key)

    if expectation_range is None:
        return 0.5, method_name

    threshold, expected = expectation_range
    ...
    score = _calculate_metric_quality(
        current_value,
        expected,
        threshold,
        higher_is_better,
