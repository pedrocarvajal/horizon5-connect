...
class AccountUpdateModel(BaseModel):
    ...
    api_key: Optional[str] = Field(
        default=None, min_length=1, description="API key (will be encrypted server-side)", repr=False
    )
    api_secret: Optional[str] = Field(
        default=None, min_length=1, description="API secret (will be encrypted server-side)", repr=False
    )
